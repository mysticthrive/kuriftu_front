...
import { 
  ...
  CreateRoomGroupData,
  UpdateRoomGroupData 
} from '@/lib/api/roomGroups';
import Pagination from '@/components/Pagination';

export default function RoomGroupPage() {
  ...
  const [deletingGroup, setDeletingGroup] = useState<RoomGroup | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.response?.data?.errors) {
        ...
        toast.error(error.response?.data?.message || 'Failed to save room group');
      }
    } finally {
      setSubmitting(false);
    }
  ...
  const handleDeleteClick = (group: RoomGroup) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || 'Failed to delete room group');
    } finally {
      setDeleting(false);
    }
  };
  ...
  // Pagination logic
  const totalItems = filteredRoomGroups.length;
  const totalPages = Math.ceil(totalItems / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const paginatedRoomGroups = filteredRoomGroups.slice(startIndex, endIndex);
                    ...
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                      ...
                      <div className="mb-4">
                        <nav className="flex items-center text-sm text-gray-500">
                          ...
                          <span className="text-gray-900 font-medium">Room Groups</span>
                        </nav>
                      </div>
               ...
               {roomGroupsLoading ? (
                ...
                <div className="p-8 text-center">
                  ...
                  <p className="text-gray-500 mb-4">
                    {searchTerm ? 'Try adjusting your search terms.' : 'Get started by creating your first room group.'}
                  </p>
                  {!searchTerm && (
                    <button
                      onClick={() => setShowModal(true)}
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {paginatedRoomGroups.map((group) => (
                        <tr key={group.room_group_id} className="hover:bg-gray-50">
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-semibold text-gray-900">
                {editingGroup ? 'Edit Room Group' : 'Create Room Group'}
              </h2>
              <button
         ...
         <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
           <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4 transform transition-all">
             <div className="flex items-center mb-4">
               ...
               <div>
                 <h2 className="text-xl font-semibold text-gray-900">Delete Room Group</h2>
             ...
             <div className="mb-6">
               <p className="text-gray-700 mb-2">
                 Are you sure you want to delete the room group:
               </p>
               <div className="bg-gray-50 rounded-lg p-3 border border-gray-200">
                 <p className="font-medium text-gray-900">{deletingGroup.group_name}</p>
                 {deletingGroup.description && (
                   <p className="text-sm text-gray-600 mt-1">{deletingGroup.description}</p>
                 )}
               </div>
             </div>
